...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function playIngame() {
    ...
    bgmIngame.currentTime = 0;
    bgmIngame.volume = getBgmVol();
    const p = bgmIngame.play();
    if (p && p.catch) p.catch(e => console.warn('BGM play failed:', e));
  }
  function stopAll() {
    bgmTitle?.pause();
    bgmIngame?.pause();
  ...
  const SKILLS = [
    ...
    { id: 'fluid',      name: '유체화',    desc: '3s 피해 50% 감소 + 이속 1.5배',            cooldown: 12000 },
    ...
    { id: 'bullettime', name: '불릿타임',  desc: '3s 탄·보스 80% 감속, 내 이속·공속 유지',   cooldown: 16000 },
  ];
  ...
  const skins = [
    ...
    { color: '#ff9944', label: '주황', cost: 14000, skill: 'bullettime' },
    { color: '#4fd3ff', label: '청록', cost: 18000, skill: 'bullettime' },
    { color: '#ffffff', label: '백색', cost: 25000, skill: 'teleport'   },
  ];
  ...
  let bulletTimeScale = 1.0;   // 불릿타임: 1.0=정상, 0.2=80%감속
  let bulletTimeMs    = 0;     // 불릿타임 잔여 시간
  ...
  class Bullet {
    ...
    update(dt) {
      ...
      const btScale = (typeof bulletTimeScale !== 'undefined') ? bulletTimeScale : 1.0;
      this.x += this.vx * btScale * dt / FRAME_REF;
      this.y += this.vy * btScale * dt / FRAME_REF;
      this.lifeMs -= dt;
  ...
  class PlayerBullet {
    ...
    draw() {
      ...
      ctx.save();
      // 글로우: shadowBlur 대신 반투명 큰 원 + 중심 원
      ctx.globalAlpha = 0.3;
      ctx.fillStyle = isOvercharge ? '#ffb0b0' : '#99aaff';
      ctx.beginPath();
      ctx.arc(this.x, this.y, isOvercharge ? 9 : 6, 0, Math.PI*2);
      ctx.fill();
      ctx.globalAlpha = 1;
  ...
  class Boss {
    ...
    resolveWaterHealing() {
      ...
      if (remain === 0) {
        ...
        spawnShockwave(player.x, player.y, 80, '#8fe7ff', 4);
        spawnShockwave(player.x, player.y, 40, '#ffffff', 2.5);
      } else {
        // 생존 토템 수만큼 보스 체력/maxHp 증가
    ...
    update(dt) {
      ...
      // 보스 데미지 — 이 아래 모든 코드에서 사용하므로 최상단 선언
      const timeDmgScale = 1.0 + score / 8000;
      ...
      if (this.phase === 'burst') {
        ...
        const len = Math.hypot(dx,dy) || 1;
        this.x += (dx/len)*this.data.chaseSpeed*(this._speedBoost||1)*bulletTimeScale*dt/1000 + Math.sin(this.moveAngle*3)*30*dt/1000;
        this.y += (dy/len)*this.data.chaseSpeed*(this._speedBoost||1)*bulletTimeScale*dt/1000 + Math.cos(this.moveAngle*2)*20*dt/1000;
        ...
        const len = Math.hypot(dx,dy) || 1;
        this.x += (dx/len)*this.data.zigSpeed*(this._speedBoost||1)*bulletTimeScale*dt/1000 + Math.sin(this.moveAngle*5)*55*dt/1000;
        this.y += (dy/len)*this.data.zigSpeed*(this._speedBoost||1)*bulletTimeScale*dt/1000 + Math.cos(this.moveAngle*4)*35*dt/1000;
        this.x = clamp(this.x, 80, canvas.width-80);
        this.y = clamp(this.y, 80, canvas.height-80);
  ...
  function spawnWallBarrage(dmg) {
    ...
    if (isHoriz) {
      ...
      for (let i = 0; i < count; i++) {
        const x = i * gap + r;
        // 전부 destructible — 플레이어 탄으로 길 뚫기
        bullets.push(new Bullet(x, startY, 0, vy, r, '#ffe566', dmg, true));
      }
    } else {
      ...
      for (let i = 0; i < count; i++) {
        const y = i * gap + r;
        bullets.push(new Bullet(startX, y, vx, 0, r, '#ffe566', dmg, true));
      }
    }
  }
    ...
    for (const orb of electricOrbs) {
      ...
      if (Math.hypot(orb.x - player.x, orb.y - player.y) < orb.r + player.r) {
        ...
        if (orb.hitCooldown <= 0) {
          orb.hitCooldown = 400;
          applyPlayerHit(bossDmg, orb.x, orb.y, true);
  ...
  function triggerElectricLinks() {
    ...
    for (const link of newLinks) {
      if (segmentCircleIntersect(link.a.x,link.a.y, link.b.x,link.b.y, player.x,player.y, player.r+6)) {
        applyPlayerHit(bossDmg, (link.a.x+link.b.x)/2, (link.a.y+link.b.y)/2, true);
  ...
  function drawElectricOrbs() {
    ...
    // Orbs — shadowBlur 없이 두 겹 원으로 글로우 대체
    for (const orb of electricOrbs) {
      if (!orb.alive) continue;
      ctx.save();
  ...
  // ─── Skill execution ──────────────────────────────────────────────
  function useSkill(skillId, source='keyboard') {
    if (gameOver) return;
    ...
    if (skillId === 'teleport') {
      ...
      if (source === 'mouse') {
        // 우클릭: 마우스 방향으로 텔포 (사거리 내면 마우스 위치, 밖이면 최대 사거리)
        const dx = mouseX - player.x, dy = mouseY - player.y;
        const dist = Math.hypot(dx, dy) || 1;
        const actualDist = Math.min(dist, TELEPORT_RANGE);
        player.x = clamp(player.x + (dx/dist)*actualDist, player.r, canvas.width  - player.r);
        ...
        player.x = clamp(player.x + (kx/klen)*TELEPORT_RANGE, player.r, canvas.width  - player.r);
        player.y = clamp(player.y + (ky/klen)*TELEPORT_RANGE, player.r, canvas.height - player.r);
      }
      dashTrailMs = 200;
  ...
  function update(dt) {
    ...
    if (bulletTimeMs > 0) {
      bulletTimeMs -= dt;
      if (bulletTimeMs <= 0) { bulletTimeMs = 0; bulletTimeScale = 1.0; }
    }
    ...
    const fluidSpeed = (skillActiveMs['fluid']||0) > 0 ? 1.5 : 1.0;
    const speedMult = (powerUpMs > 0 ? POWERUP_SPEED_MULT : 1) * windDebuff * fluidSpeed;
    ...
    // keyMode: 'wasd_attack'=화살표이동+WASD공격, 'wasd_move'=WASD이동+화살표공격
    ...
    if (moveDown)  player.y += move;
    player.x = clamp(player.x, player.r, canvas.width -player.r);
    player.y = clamp(player.y, player.r, canvas.height-player.r);

    // 공격 키
    const atkL = wasdIsMove ? keys['ArrowLeft']  : keys['a'];
    const atkR = wasdIsMove ? keys['ArrowRight'] : keys['d'];
    const atkU = wasdIsMove ? keys['ArrowUp']    : keys['w'];
    const atkD = wasdIsMove ? keys['ArrowDown']  : keys['s'];
    const kbFireX = player.x + ((atkR?1:0) - (atkL?1:0)) * 999;
    const kbFireY = player.y + ((atkD?1:0) - (atkU?1:0)) * 999;
    const kbFiring = atkL || atkR || atkU || atkD;
    if (kbFiring) firePlayerBullet(kbFireX, kbFireY);
    ...
    if (bossActive && boss) {
      ...
      if (boss.hp <= 0) {
        ...
        electricOrbs=[]; electricLinks=[];
        windMinions=[]; windSlowActive=false;
    bulletTimeScale=1.0; bulletTimeMs=0;
        lightningStrikes=[];
        bullets=[];
        if (bossUi) bossUi.style.display='none';
      }
  ...
  // ─── Draw ─────────────────────────────────────────────────────────
  function isLight() { return _lightModeCache; }
  ...
  // ─── Game loop ────────────────────────────────────────────────────
  let _lightModeCache = false;
  let _keyMode = localStorage.getItem('keyMode') || 'wasd_attack';
  // 설정 변경 감지 (storage 이벤트 — 다른 탭이나 메인메뉴에서 변경 시)
  window.addEventListener('storage', e => {
    if (e.key === 'keyMode') _keyMode = e.newValue || 'wasd_attack';
  });
  ...
  function loop(time) {
    ...
    lastTime = time;
    _lightModeCache = document.body.classList.contains('light');
    _keyMode = localStorage.getItem('keyMode') || 'wasd_attack';
  ...
  function togglePause() {
    ...
    if (isPaused) {
      ...
      lastTime = performance.now(); // dt 튀는 거 방지
    }
  }
  ...
  document.addEventListener('click', () => {
    // 메인메뉴가 표시 중일 때만 타이틀 BGM 시작
    if (menu && menu.style.display !== 'none') playTitle();
  }, { once: true });
});
