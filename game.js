...
document.addEventListener('DOMContentLoaded', () => {
  ...
  class Boss {
    ...
    update(dt) {
      ...
      // 보스별 기본 데미지 — windAura/waterDrain/전기 스킬보다 먼저 선언
  ...
  function initGame() {
    ...
    renderSkillButtons();
    this._hpTimer = (this._hpTimer||0) + dt;
    if (this._hpTimer >= 50) { this._hpTimer = 0; updateHpBar(); }
  ...
  function update(dt) {
    ...
    // 스킬 HUD는 100ms마다만 갱신 (DOM 조작 빈도 감소)
    this._skillHudTimer = (this._skillHudTimer||0) + dt;
    if (this._skillHudTimer >= 100) { this._skillHudTimer = 0; updateSkillHud(); }
