...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const SKILLS = [
    ...
    { id: 'hydroheal',  name: '수류치유',  desc: '물 보스전 한정 — HP 30 즉시 회복',          cooldown: 8000  },
  ];
  ...
  const skins = [
    ...
    { color: '#4fd3ff', label: '청록', cost: 18000, skill: 'hydroheal'  },
  ...
  function useSkill(skillId) {
    ...
    if (skillId === 'hydroheal') {
      // 물 보스전 한정 — 아닌 경우 쿨 소모 없이 취소
      if (!bossActive || !boss || !boss.data.waterHealOnThreshold) {
        skillCooldowns[skillId] = 0;
        return;
      }
      player.hp = Math.min(player.maxHp, player.hp + 30);
      spawnShockwave(player.x, player.y, 35, '#4fd3ff', 2.5);
      return;
    }
